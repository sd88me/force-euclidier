...
## Fork notes (sd88me/EUCLIDIER-CONSOLE, branch `force-rework`)

Changes for the Akai Force, on top of upstream 0.1.9 (see `DESIGN.md` for the full write-up):
- **No resync needed**: step position is derived from the transport tick count, so editing steps/fill/shift/loop/division stays locked to the beat. Song position (0xF2) and MIDI Continue are handled.
- **CPU**: ~5.3% -> ~0.4% while playing (event-driven main loop). Idle 0%.
- **Control socket** (`/tmp/euclidier_ctrl.sock`, `--ctrl-sock PATH`): `GET`/`SET` text protocol; used by the shadow-mode GUI. Every SET goes through the same code as the MIDI CC interface, which is unchanged.
- **Shadow-mode GUI** for [force-shadow](https://github.com/sd88me/force-shadow): `addon/shadow_page.conf`, generated by `scripts/gen_shadow_page.py`. Needs force-shadow's `euclid` widget.
- Build for the Force (armhf, Docker): `build/build.sh`.
